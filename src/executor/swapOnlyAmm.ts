...
import { RPC_ENDPOINT, solanaConnection, TX_FEE, WEBSOCKET_SUB_ENDPOINT } from "../config/config"
...
export async function getSellTx(wallet: Keypair, baseMint: PublicKey, quoteMint: PublicKey, amount: string, targetPool: string) {
    try {
        ...
        console.log("🚀 ~ getSellTx ~ willSendTx:", willSendTx)
        if (willSendTx instanceof VersionedTransaction) {
            willSendTx.sign([wallet])
            return willSendTx
        }
...
export const getSellTxWithJupiter = async (wallet: Keypair, baseMint: PublicKey, amount: string) => {
    try {
        const quoteResponse = await (
            await fetch(
                `https://quote-api.jup.ag/v6/quote?inputMint=${baseMint.toBase58()}&outputMint=So11111111111111111111111111111111111111112&amount=${amount}&slippageBps=${50}`
            )
        ).json();
...
export const execute = async (transaction: VersionedTransaction, latestBlockhash: Blockhash, isBuy: boolean | 1 = true) => {
    const solanaConnection = new Connection(RPC_ENDPOINT, {
        wsEndpoint: WEBSOCKET_SUB_ENDPOINT,
    })
